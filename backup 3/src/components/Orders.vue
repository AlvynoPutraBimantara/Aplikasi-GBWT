  ...
  return `http://localhost:3003/${cleanUrl}?t=${timestamp}`;
    ...
    async renderWithPdfJs(order) {
      try {
        ...
        // Fetch the PDF with arraybuffer response type
    ...
    async acceptOrder(order) {
      try {
        ...
        const transactionData = {
          id: order.id, // Use the same ID as the order
          user: user.id,
          total: order.total,
        ...
        // Delete the order using the new endpoint
        try {
          await axios.delete(`http://localhost:3003/orders/${order.id}/complete`);
        } catch (deleteError) {
          console.error("Order deletion failed, but transaction was created:", deleteError);
          // We can continue even if deletion fails since transaction was created
        }
