...
export default {
  ...
  try {
    ...
    // Extract order ID from URL if it's in the format /invoices/ORDER_ID/FILENAME
    ...
    if (invoiceUrl.startsWith('http')) {
      ...
      fullUrl = `http://localhost:3003/invoices/${orderId}/${filename}`;
    } else {
      // Fallback to direct URL
      fullUrl = `http://localhost:3003${invoiceUrl}`;
    }

    // Add timestamp to prevent caching
    fullUrl += (fullUrl.includes('?') ? '&' : '?') + 't=' + new Date().getTime();
